...
export default function JourneyRow({ step, index, clientPayment, projectId, isLast }) {
...
const pillLabel = money
  ...
  return (
    <div
      className={`flex items-center justify-between gap-4 px-4 transition-colors hover:bg-white/[0.03] ${
        isDone ? 'py-2' : 'py-3'
      } ${!isLast ? 'border-b border-white/[0.06]' : ''} ${error ? 'bg-danger/5' : ''}`}
    >
        ...
        <div className='flex flex-col min-w-0'>
          ...
          {isMilestone && !isDone && (
            editingDueDate ? (
              ...
              <span className='flex items-center gap-1 font-mono text-[12px] mt-1'>
          ...
          {dLabel && (
            editingDate ? (
              ...
              <span className={`flex items-center gap-1 font-mono text-[12px] mt-1 ${isDone ? 'text-white/25' : 'text-warning'}`}>
                {dLabel}
                {canEditDate && (
                  <button
                    type='button'
  ...
  <button
    ...
    disabled={isPending}
    className='flex items-center gap-1.5 font-mono text-xs uppercase tracking-wide disabled:opacity-50'
  >
  ...
  <div className='relative' ref={menuRef}>
    ...
    >
      <span className={`font-mono text-xs uppercase tracking-wide ${statusPillClass(displayStatus, isDone)}`}>
        {pillLabel}
      </span>
    ...
    {menuOpen && (
              <div className='absolute right-0 top-full mt-1 z-20 bg-dark border border-white/10 rounded-lg py-1 min-w-32 shadow-xl'>
                {JOURNEY_STATUS_ORDER.map((s) => (
                  <button
                    key={s}
                    type='button'
                    onClick={() => handleStatusPick(s)}
                    className={`block w-full text-left px-3 py-1.5 font-mono text-xs uppercase tracking-wide hover:bg-white/5 ${
                  ...
                  <div className='border-t border-white/10 mt-1 pt-1'>
                    {WAITING_ON_OPTIONS.map((w) => (
                      <button
                        ...
                        className={`block w-full text-left px-3 py-1.5 font-mono text-xs hover:bg-white/5 ${
