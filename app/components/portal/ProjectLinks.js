...
const BUILD_COMPANION_URL =
	'https://claude.ai/project/01a05e56-0098-7153-a932-838649e42136';
...
export default function ProjectLinks({
	...
	const keys = editable
		? links[variant]
		: links[variant].filter((key) => urls[key]);
	...
	// If the touch was held long enough to reveal the pencil, swallow the
	// click that follows on release so it doesn't also navigate the pill's link.
			...
			{keys.map((key) => {
					...
					return (
						<div key={key} className='flex flex-col gap-1'>
							<div className='flex items-center gap-1.5 bg-white/[0.04] border border-teal/40 rounded-xl pl-3 pr-1.5 py-1.5'>
								...
								<button
									onClick={() => save(key)}
									className='text-teal hover:text-teal/70 p-1'
									aria-label='Save'
								>
									<TbCheck size={15} />
								</button>
							...
							{editError && (
								<span className='text-[11px] text-danger pl-1'>
									{editError}
								</span>
							)}
						</div>
				...
				return (
					<div
						...
						onTouchStart={canEdit ? () => handleTouchStart(key) : undefined}
						onTouchEnd={canEdit ? cancelLongPress : undefined}
						onTouchMove={canEdit ? cancelLongPress : undefined}
						onClickCapture={canEdit ? handleClickCapture : undefined}
					>
						...
						{canEdit && (
							<button
								...
								className={`absolute -top-1.5 -right-1.5 z-10 p-2.5 -m-2.5 touch-manipulation transition-opacity duration-200 opacity-0 lg:group-hover:opacity-100 ${
									revealedKey === key ? 'opacity-100' : ''
								}`}
							>
